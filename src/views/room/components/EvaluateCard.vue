...
const score = ref(0) // 评价分数
const anonymousFlag = ref(false) // 是否开启匿名评价
const content = ref('') // 评价内容
const disabled = computed(() => !score.value || !content.value) // 是否禁用
...
const onSubmit = async () => {
  ...
  if (consult.value.docInfo?.id) {
    const res = await evaluateConsultOrder({
